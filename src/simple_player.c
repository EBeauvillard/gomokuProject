...
//TODO : trouver pourquoi n_moves > 0 fait des erreurs sur valgrind
struct move_t play(struct col_move_t const previous_moves[], size_t n_moves)
{
  ...
  if(p.color == BLACK)
  {
    move = simple_strat(*(p.bitboardB), *(p.bitboardW), p.size);
    *(p.bitboardB) = operation_on_token(*(p.bitboardB),move.row,move.col,p.size,'+');
  }
  else
  {
    move = simple_strat(*(p.bitboardW), *(p.bitboardB), p.size);
    *(p.bitboardW) = operation_on_token(*(p.bitboardW),move.row,move.col,p.size,'+');
  }
    /*
    printf("simple : \n");
